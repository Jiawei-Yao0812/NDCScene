...
from ndcscene.models.DDR import Bottleneck3D
...
class LightSegmentationHead(nn.Module):
    ...
    def __init__(self, planes, nbr_classes, dilations_conv_list):
        super().__init__()
        # ASPP Block
        self.conv_list = dilations_conv_list
        self.conv1 = nn.ModuleList(
            [
                nn.Conv3d(
                    planes, planes, kernel_size=3, padding=dil, dilation=dil, bias=True
                )
                for dil in dilations_conv_list
            ]
        )
    ...
    def forward(self, x_in):
        ...
        # Convolution to go from inplanes to planes features...
        y = sum([self.bn2[i](self.conv2[i](self.relu(self.conv1[i](x_in)))) for i in range(len(self.conv_list))])
        x_in = self.relu(y + x_in)  # modified
    ...
    def __init__(self, feature, norm_layer, bn_momentum, expansion=8):
        ...
        self.main = Bottleneck3D(
            ...
            downsample=nn.Sequential(
                ...
                norm_layer(feature * expansion // 4, momentum=bn_momentum),
            ),
            norm_layer=norm_layer,
        )
