...
class TSDFVolume:
    ...
    @staticmethod
    @njit(parallel=True)
    def vox2world(vox_origin, vox_coord, vox_size, offsets=(0.5, 0.5, 0.5)):
        """Convert voxel grid coordinates to world coordinates."""
        vox_origin = vox_origin.astype(np.float32)
        vox_coord = vox_coord.astype(np.float32)
        #    print(np.min(vox_coords))
        cam_pts = np.empty_like(vox_coord, dtype=np.float32)

        for i in prange(vox_coord.shape[0]):
            for j in range(3):
                cam_pts[i, j] = (
                    vox_origin[j]
                    + (vox_size * vox_coord[i, j])
                    + vox_size * offsets[j]
                )
        return cam_pts
