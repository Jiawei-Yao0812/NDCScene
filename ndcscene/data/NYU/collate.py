...
def collate_fn(batch):
    ...
    ret_data["projected_pix_1"] = torch.stack(ret_data["projected_pix_1"]).to(torch.float32)
    ret_data["fov_mask_1"] = torch.stack(ret_data["fov_mask_1"]).to(torch.bool)
    ret_data["CP_mega_matrices"] = torch.stack(ret_data["CP_mega_matrices"]).to(torch.bool)
    return ret_data
